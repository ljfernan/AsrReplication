...
LogTrace("[START]-Cleanup failover")
LogTrace("File: $($CsvFilePath)")

$resolvedCsvPath = Resolve-Path -LiteralPath $CsvFilePath
$csvObj = Import-Csv $resolvedCsvPath -Delimiter ','

$CsvOutput = [string]::Concat($resolvedCsvPath.Path, ".cleanupfailover.", (Get-Date).ToString("ddMMyyyy_HHmmss"), ".output.csv")
...
class TestFailOverCleanUpInformation
{
    [string]$Machine
    [string]$Exception
    [string]$TestFailoverCleanUpJobId
...
Function GetProtectedItemStatus($csvItem)
{
    ...
    $statusItemInfo = [TestFailOverCleanUpInformation]::new()
    ...
    if ($protectableVM.ReplicationProtectedItemId -ne $null)
    {
        ...
        if ($protectedItem.AllowedOperations.Contains('TestFailoverCleanup'))
        {
            ...
            $testFailoverCleanUpJob = Start-ASRTestFailoverCleanupJob `
                -ReplicationProtectedItem $protectedItem

            $statusItemInfo.TestFailoverCleanUpJobId = $testFailoverCleanUpJob.ID
        } else {
            LogTrace "TestFailoverCleanup operation not allowed for item '$($sourceMachineName)'"
...
foreach ($csvItem in $csvObj)
{
    try {
        ...
        LogError "Exception executing process"
...
LogTrace("[FINISH]-Cleanup failover")
